...
public class ViewModelBase : SelfProvidingMarkupExtension, INotifyPropertyChanged, IRaisePropertyChanged
{
    ...
    /// <summary>
    /// Contains Command Dependencies, values will be called when the key value has changed
    /// </summary>
    public Dictionary<string, List<string>> CommandDependencies { get; } = new Dictionary<string, List<string>>();
    /// <summary>
    ...
    private void ViewModelBase_PropertyChanged(object sender, PropertyChangedEventArgs e)
    {
        ...
        if (CommandDependencies.TryGetValue(e.PropertyName, out var cresult))
        {
            foreach (var item in cresult)
            {
                if(Values.TryGetValue(item, out var value) && value is Command ic)
                {
                    ic.ChangeCanExecute();
                }
            }
    ...
    public void AddCommandDependency(string source, params string[] dependencies)
    {
        var exists = CommandDependencies.TryGetValue(source, out var result);
        if (!exists)
            CommandDependencies.Add(source, result = new List<string>());
    ...
    internal void AddCommandSupplyer(string source, string[] suppliers)
    {
        foreach (var item in suppliers)
        {
            AddCommandDependency(item, source);
        } 
    }
    /// <summary>
